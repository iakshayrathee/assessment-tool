                  ...
                  {document.weeklyEvaluations.flatMap((evaluation: any) =>
                    evaluation.activities?.map((activity: any, idx: number) => (
                      <tr key={`${evaluation.id}-${idx}`} className="hover:bg-gray-50">
                        ...
                        <td className="border border-gray-300 p-3 text-center">{activity.behavioralAttention}</td>
                        <td className="border border-gray-300 p-3 text-center">{activity.behavioralSittingTolerance}</td>
                        <td className="border border-gray-300 p-3 text-center">{activity.behavioralTaskCompletion}</td>
                      </tr>
                    ))
                  )}
                </tbody>
              ...
              {document.weeklyEvaluations.map((evaluation: any) => (
                <div key={evaluation.id}>
                  <h3 className="text-lg font-semibold mb-3">
                    Week of {formatDate(evaluation.weekStartDate)}
                  </h3>
                  <div className="overflow-x-auto">
                    <table className="w-full border-collapse border border-gray-300 text-sm">
                      <thead>
                        <tr className="bg-gray-100">
                          <th className="border border-gray-300 p-2 text-left font-semibold rotate-180" style={{ writingMode: 'vertical-rl' }}>
                            Categories
                          </th>
                          <th className="border border-gray-300 p-2 text-left font-semibold rotate-180" style={{ writingMode: 'vertical-rl' }}>
                            Specific Objectives
                          </th>
                          <th className="border border-gray-300 p-2 text-center font-semibold">Monday</th>
                          <th className="border border-gray-300 p-2 text-center font-semibold">Tuesday</th>
                          <th className="border border-gray-300 p-2 text-center font-semibold">Wednesday</th>
                          <th className="border border-gray-300 p-2 text-center font-semibold">Thursday</th>
                        </tr>
                      </thead>
                      <tbody>
                        {evaluation.activities && evaluation.activities.length > 0 ? (
                          evaluation.activities.map((activity: any, idx: number) => (
                            <tr key={idx} className="hover:bg-gray-50">
                              <td className="border border-gray-300 p-2 font-medium">{activity.subject}</td>
                              <td className="border border-gray-300 p-2">{activity.testGoalActivity}</td>
                              <td className="border border-gray-300 p-2">{activity.dayOfWeek === 'MONDAY' ? activity.analysis : ''}</td>
                              <td className="border border-gray-300 p-2">{activity.dayOfWeek === 'TUESDAY' ? activity.analysis : ''}</td>
                              <td className="border border-gray-300 p-2">{activity.dayOfWeek === 'WEDNESDAY' ? activity.analysis : ''}</td>
                              <td className="border border-gray-300 p-2">{activity.dayOfWeek === 'THURSDAY' ? activity.analysis : ''}</td>
                            </tr>
                          ))
                        ) : (
                          <tr>
