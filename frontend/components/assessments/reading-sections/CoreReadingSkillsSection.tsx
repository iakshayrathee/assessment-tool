...
const EXPRESSION_OPTIONS = ['Flat', 'Moderate', 'Good'];
const PAUSING_OPTIONS = ['Correct', 'Incorrect'];
  ...
  return (
    <div className="space-y-1.5">
      <div className="flex justify-between items-center">
        ...
        <span className="text-xs font-medium text-primary">{value ?? '—'}/{max}</span>
      </div>
      <Slider
    ...
    <div className="space-y-4">
      ...
      <Card>
        <CardHeader className="pb-3">
          <CardTitle className="text-base">A. Phonological Awareness</CardTitle>
        </CardHeader>
        <CardContent className="space-y-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <ScoreSlider label="Blending" value={phon.blending} onChange={(v) => updatePhon('blending', v)} disabled={disabled} />
            <ScoreSlider label="Segmenting" value={phon.segmenting} onChange={(v) => updatePhon('segmenting', v)} disabled={disabled} />
            <ScoreSlider label="Sound Identification" value={phon.soundIdentification} onChange={(v) => updatePhon('soundIdentification', v)} disabled={disabled} />
          </div>
        </CardContent>
      </Card>
      ...
      <Card>
        <CardHeader className="pb-3">
          <CardTitle className="text-base">B. Decoding</CardTitle>
        </CardHeader>
        <CardContent>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <ScoreSlider label="Letter-Sound Knowledge" value={dec.letterSoundKnowledge} onChange={(v) => updateDec('letterSoundKnowledge', v)} disabled={disabled} />
            <ScoreSlider label="CVC Words" value={dec.cvcWords} onChange={(v) => updateDec('cvcWords', v)} disabled={disabled} />
            <ScoreSlider label="Blends/Digraphs" value={dec.blendsDigraphs} onChange={(v) => updateDec('blendsDigraphs', v)} disabled={disabled} />
            <ScoreSlider label="Multisyllabic Decoding" value={dec.multisyllabicDecoding} onChange={(v) => updateDec('multisyllabicDecoding', v)} disabled={disabled} />
          </div>
        </CardContent>
      </Card>
      ...
      <Card>
        <CardHeader className="pb-3">
          <CardTitle className="text-base">C. Fluency</CardTitle>
        </CardHeader>
        <CardContent>
      ...
      <Card>
        <CardHeader className="pb-3">
          <CardTitle className="text-base">D. Sight Words</CardTitle>
        </CardHeader>
        <CardContent>
          <div className="max-w-xs">
            <Label>% Correct from Grade List</Label>
            <Input type="number" min={0} max={100} step={0.1}
      ...
      <Card>
        <CardHeader className="pb-3">
          <CardTitle className="text-base">E. Mechanics</CardTitle>
        </CardHeader>
        <CardContent>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div className="space-y-2">
              <Label>Punctuation Awareness</Label>
              <div className="flex gap-4">
                <label className="flex items-center gap-2 cursor-pointer">
            ...
            <div>
              ...
              <Select value={data.readingExpression || ''} onValueChange={(v) => onChange({ readingExpression: v })} disabled={disabled}>
                ...
                <SelectContent>{EXPRESSION_OPTIONS.map(o => <SelectItem key={o} value={o}>{o}</SelectItem>)}</SelectContent>
              </Select>
            </div>
            <div>
              <Label>Pausing</Label>
              <Select value={data.pausingCorrectness || ''} onValueChange={(v) => onChange({ pausingCorrectness: v })} disabled={disabled}>
                <SelectTrigger className="mt-1"><SelectValue placeholder="Select" /></SelectTrigger>
                <SelectContent>{PAUSING_OPTIONS.map(o => <SelectItem key={o} value={o}>{o}</SelectItem>)}</SelectContent>
              </Select>
            </div>
          </div>
        </CardContent>
      ...
      <Card>
        <CardHeader className="pb-3">
          <CardTitle className="text-base">F. Visual Tracking</CardTitle>
        </CardHeader>
        <CardContent>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            {[
              { key: 'skipsLinesVisual' as const, label: 'Skips Lines' },
              { key: 'usesFinger' as const, label: 'Uses Finger' },
              { key: 'losesPlace' as const, label: 'Loses Place' },
            ].map(({ key, label }) => (
